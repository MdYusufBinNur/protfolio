...
<script>
    export default {
        ...
        methods:{
            ...
            loadCategories()
            {
                axios.get('api/blog_categories').then(({data})=>(this.categories = data.data));
            },
            ...
            deleteCategory(category)
            {
                swal.fire({
                    ...
                    if (result.value) {
                        // Send HTTP Request
                        this.form.delete('api/blog_categories/'+category).then(() => {
            ...
            {
                this.$Progress.start();
                this.form.post('api/blog_categories').then(() => {
                    Fire.$emit('afterCreate');
                    toast.fire({
                        icon: 'success',
            ...
            {
                this.$Progress.start();
                this.form.put('api/blog_categories/'+ this.form.id).then(()=>{
                    Fire.$emit('afterCreate');
                    this.$Progress.finish();
                    toast.fire({
                        icon: 'success',
